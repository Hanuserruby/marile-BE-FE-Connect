...
function LandingPage() {
  ...
  useEffect(() => {
    api.get('/public/menu?category=protein')
      .then((res) => {
  ...
  return (
    <div>
      ...
      <section className="best-seller">
        {loading ? (
          <p style={{ color: 'white', textAlign: 'center' }}>Memuat...</p>
        ) : bestSeller.length === 0 ? (
          <p style={{ color: 'white', textAlign: 'center' }}>Belum ada data best seller.</p>
        ) : (
          bestSeller.slice(0, 3).map((product, index) => (
            <div className={`best-item ${index === 1 ? 'center' : 'side'}`} key={product.product_id}>
              <img
                src={product.image_url || "https://images.unsplash.com/photo-1615141982883-c7ad0e69fd62?w=400"}
                alt={product.product_name}
              />
              <div className="badge" style={index === 1 ? { padding: '10px 50px' } : {}}>
      ...
      <section className="menu-section">
        <div className="menu-grid">
          {loading ? (
            <p style={{ color: 'white', textAlign: 'center', gridColumn: '1 / -1' }}>Memuat menu...</p>
          ) : menu.length === 0 ? (
